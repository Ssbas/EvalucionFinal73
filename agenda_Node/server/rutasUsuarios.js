...
Router.get('/sebas', function(req, res) {
  Usuarios.find({user: req.query.user}).count({}, function(err, count) {
    if(count>0){
        res.send("Utilice los siguientes datos: </br>usuario: sebas | password:123456 </br>usuario: juan | password:123456")
    }else{
      ...
      Operaciones.crearUsuariosebas((error, result) => {
        if(error){
          res.send(error)
        }else{
          res.send(result)
